...
def generate_unique_physics(category_key):
    ...
    elif category_key == "stars": return "Main Sequence Star", f"Mass: {round(random.uniform(0.5, 40.0), 1)} M_sun, Core Temp: {random.randint(5000, 45000)} K"
    ...
    elif category_key == "neutron_stars": return "Pulsar", f"B-Field: 10^{random.randint(11, 15)} Gauss, Spin Period: {round(random.uniform(1.2, 85.0), 1)} ms"
...
def run_physics_tick(all_objects, current_age):
    ...
    for obj in all_objects:
        ...
        if "Star" in obj_type or obj_type == "Main Sequence":
            ...
            # REALISM FIX: Dramatically slower hydrogen burn rate so stars live longer
            burn_rate = (mass ** 2.5) * 0.0005 
            ...
            if not has_life and 270 <= temp <= 350:
                abio_index = float(obj.get("abiogenesis_index") or 0.0) + random.uniform(0.01, 0.03)
            ...
            if has_life:
                progress = float(obj.get("progress_index") or 0.0) + random.uniform(0.1, 0.3)
...
def run_expansion_step(state, stats):
    ...
    # REALISM FIX: Weighted probability. Common things spawn often, rare things are actually rare.
    possible_spawns = ["nebulae"]
    weights = [15] 
    
    if c_nebulae >= 1: 
        possible_spawns.extend(["stars", "asteroids_comets"])
        weights.extend([40, 60]) 
    if c_stars >= 5: 
        possible_spawns.extend(["planets", "moons"])
        weights.extend([50, 70])
    if c_stars >= 15: 
    ...
    # Select one category based on the realistic weights
    cat_key = random.choices(possible_spawns, weights=weights, k=1)[0]
    ...
    print(f"✨ [EXPANSION]: Age {current_age} Gyr | Spawned: {ai_name} ({cat_label})", flush=True)
    ...
    db_post("celestial_objects", {
        ...
        "z_coord": z,
        "is_dead": False,
        "hydrogen_pct": 100.0 # Make sure new objects start fresh
    })
...
if __name__ == "__main__":
    print(f"🚀 [ORIGIN RUNNER ENGINE] Online. Realistic Physics Mode...", flush=True)
    t_math, t_expand = 0, 0
